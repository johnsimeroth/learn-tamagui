...
## Inline Styling
This probably isn't your first rodeo when it comes to inline styling, so I'll keep this short. Inline styling is great because it makes it really clear where your styles are being applied, and removes the need to wire up classnames. However, it's gotten a bad rap in terms of performance, for good reason. Tamagui enables you to write inline styles, without the performance trade offs, because it _*replaces inline styles with pure CSS on web and does something else on native that I will look up later*_.

The way you write inline styles with tamagui is very similar to what you may have done with tailwind in the past. You pass style properties as props and set their values as desired. For example, you can add the following to your HomeScreen's XStack component:

```tsx
    <XStack borderRadius='30px' display='flex' flex={1} flexWrap='wrap' backgroundColor='#c5c5c5'>
      <H1>Hello World!</H1>
    </XStack>
```
...
## Tokens
Now that you know how to style, and style concisely, let's take a look at tokens. Tokens are more or less for sizing things. For example:
...
```tsx
    <XStack br=radius1 dsp='flex' f={1} fw='wrap' bg='$background'>
      <H1>Hello World!</H1>
    </XStack>
```

The motivation here is the same - if you wanted to update your definition of red, or your standard background color, you only have to change one line of code.

## Moving on
Well done! If you've been able to confirm that inline styles are updating your components on both web and mobile, you're ready to move on to the next lesson.
